              ...
              <div className="flex justify-between items-start mb-12">
                <div className="flex flex-col gap-1">
                  ...
                  <p className={`text-[8px] font-headline tracking-tighter pl-3 ${
                    (proj.status?.toLowerCase() === 'published' || proj.status?.toLowerCase() === 'publié') ? 'text-green-500/50' : 
                    (proj.status?.toLowerCase() === 'in progress' || proj.status?.toLowerCase() === 'in_progress' || proj.status?.toLowerCase() === 'en cours') ? 'text-orange-500/50' : 
                    (proj.status?.toLowerCase() === 'archived' || proj.status?.toLowerCase() === 'archive' || proj.status?.toLowerCase() === 'archivé') ? 'text-red-500/50' : 'text-gray-500/50'
                  }`}>STATUS: {((proj.status && proj.status.replace('_', ' ')) || 'ACTIVE').toUpperCase()}</p>
                </div>
